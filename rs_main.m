...
START_FRAME = 1;
END_FRAME = movinfo.NumberOfFrames; % WnB movinfo.NumberOfFrames;
...
VTRANS_SCALE = 100; %used
VISUAL_ODO_SHIFT_MATCH = 140; %used 
...
%aviread takes an array of indexes i.e. start:end
%videoreader.read takes 1x2 [start, end]
mov = movinfo.read([START_FRAME, BLOCK_READ+START_FRAME]);
...
for frame=2:min([END_FRAME, movinfo.NumberOfFrames])
    ...
    if (mod(frame, BLOCK_READ) == 0)
        ...
        endt = min([(frame+BLOCK_READ - 1)+START_FRAME, movinfo.NumberOfFrames]);
        %[(frame+BLOCK_READ - 1)+START_FRAME, movinfo.NumberOfFrames]]);
        mov = movinfo.read([frame+START_FRAME, endt]);
        
        %mov = aviread(MOV_FILE, (frame+START_FRAME):min([(frame+BLOCK_READ - 1)+START_FRAME, movinfo.NumberOfFrames]));
        
        if (ODO_FILE ~= 0)
            ododata = csvread(ODO_FILE, frame+START_FRAME, 0, [frame+START_FRAME 0 min([(frame+BLOCK_READ - 1)+START_FRAME, movinfo.NumberOfFrames]) 1]);
        end
        tic
    end

    % visual templates and visual odo uses intensity so convert to grayscale
    %videoreader.read returns H by W by 3 by F, this backwards to aviread
    %which returns F by H by W by 3
    imt = mov(:, :, :, mod(frame, BLOCK_READ)+1);
    %size(mov)
    %rgb2gray expects H by W by 3
    im = rgb2gray(imt);
